"""MCP tool resolution for 12FZ协作AI agents.
...
into a standalone MCPToolResolver. It handles three flavours of MCP reference:
  ...
  3. AMP references:   e.g. "notion" or "notion#search" (legacy "fzxiezuoai-amp:" prefix also works)
"""
...
class MCPToolResolver:
    """Resolves MCP server references / configs into 12FZ协作AI ``BaseTool`` instances.
    ...
    def _parse_amp_ref(mcp_config: str) -> tuple[str, str | None]:
        ...
        legacy ``"fzxiezuoai-amp:notion"`` form.
        """
        bare = mcp_config.removeprefix("fzxiezuoai-amp:")
    ...
    def _resolve_amp(
        ...
        from fzxiezuoai.events.types.mcp_events import MCPConfigFetchFailedEvent
    ...
    def _fetch_amp_mcp_configs(self, slugs: list[str]) -> dict[str, dict[str, Any]]:
        """Fetch MCP server configurations via 12FZ协作AI+ API.

        Sends a GET request to the 12FZ协作AI+ mcps/configs endpoint with
        comma-separated slugs. 12FZ协作AI+ proxies the request to fzxiezuoai-oauth.
        ...
        try:
            from fzxiezuoai_tools.tools.crewai_platform_tools.misc import (
    ...
    def _resolve_external(self, mcp_ref: str) -> list[BaseTool]:
        ...
        from fzxiezuoai.tools.base_tool import BaseTool
        from fzxiezuoai.tools.mcp_tool_wrapper import MCPToolWrapper
    ...
    def _resolve_native(
        ...
        """
        from fzxiezuoai.tools.base_tool import BaseTool
        from fzxiezuoai.tools.mcp_native_tool import MCPNativeTool
            ...
            if mcp_config.tool_filter:
                ...
                for tool in tools_list:
                    if callable(mcp_config.tool_filter):
                        try:
                            from fzxiezuoai.mcp.filters import ToolFilterContext
    ...
    def _build_mcp_config_from_dict(
        ...
        """Convert a config dict from fzxiezuoai-oauth into an MCPServerConfig."""
