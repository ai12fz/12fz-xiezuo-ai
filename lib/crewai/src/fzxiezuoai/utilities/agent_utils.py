...
from fzxiezuoai_core.printer import PRINTER, ColoredText, Printer
from fzxiezuoai_core.settings import Settings
from pydantic import BaseModel
from rich.console import Console

from fzxiezuoai.agents.constants import FINAL_ANSWER_AND_PARSABLE_ACTION_ERROR_MESSAGE
...
from fzxiezuoai.llms.base_llm import BaseLLM, call_stop_override
from fzxiezuoai.tools import BaseTool as CrewAITool
...
def convert_tools_to_openai_schema(
    ...
    """Convert 12FZ协作AI tools to OpenAI function calling format.

    This function converts 12FZ协作AI BaseTool and CrewStructuredTool objects
    into the OpenAI-compatible tool schema format that can be passed to
    LLM providers for native function calling.

    Args:
        tools: List of 12FZ协作AI tool objects to convert.
...
def _llm_stop_words_applied(
    ...
    Uses :func:`fzxiezuoai.llms.base_llm.call_stop_override` so the LLM's stop
...
def handle_context_length(
    ...
    if respect_context_window:
        ...
        if verbose:
            printer.print(
                content="Context length exceeded. Consider using smaller text or RAG tools from fzxiezuoai_tools.",
                color="red",
            )
        raise SystemExit(
            "Context length exceeded and user opted not to summarize. Consider using smaller text or RAG tools from fzxiezuoai_tools."
        )
...
def load_agent_from_repository(from_repository: str) -> dict[str, Any]:
    ...
    if from_repository:
        ...
        if callable(_create_plus_client_hook):
            ...
            from fzxiezuoai.auth.token import get_auth_token
        ...
        if response.status_code == 404:
            raise AgentRepositoryError(
                ...
                f"\nIf you are using the wrong organization, switch to the correct one using `fzxiezuoai org switch <org_id>` command.",
            )
