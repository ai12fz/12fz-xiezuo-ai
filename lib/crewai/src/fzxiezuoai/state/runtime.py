"""Unified runtime state for 12FZ协作AI.
...
The ``Entity`` type is resolved at import time in ``fzxiezuoai/__init__.py``
...
from fzxiezuoai.context import capture_execution_context
...
from fzxiezuoai.state.provider.core import BaseProvider
...
def _migrate(data: dict[str, Any]) -> dict[str, Any]:
    ...
    if raw is None:
        ...
        logger.debug(
            "Migrating checkpoint from 12FZ协作AI %s to %s",
            stored,
            current,
        )
...
class RuntimeState(RootModel):  # type: ignore[type-arg]
    ...
    def from_checkpoint(cls, config: CheckpointConfig, **kwargs: Any) -> RuntimeState:
        ...
        """
        from fzxiezuoai.state.provider.utils import detect_provider
