...
from fzxiezuoai_devtools.docs_check import docs_check
from fzxiezuoai_devtools.prompts import RELEASE_NOTES_PROMPT, TRANSLATE_RELEASE_NOTES_PROMPT
...
def update_pyproject_dependencies(
    ...
    for i, line in enumerate(lines):
        ...
        for pkg in workspace_packages:
            if pkg == "fzxiezuoai-files" and current_extra == "file-processing":
                continue
            if f"{pkg}==" in line:
...
def update_changelog(
    ...
    entry = (
        ...
        f"\n"
        f"  [{link_text}]"
        f"(https://github.com/crewAIInc/12FZ协作AI/releases/tag/{version})\n"
        f"\n"
        f"{indented_notes}\n"
        f"\n"
        f"</Update>"
...
def _is_crewai_dep(spec: str) -> bool:
    """Return True if *spec* is a ``fzxiezuoai`` or ``fzxiezuoai[...]`` dependency."""
    if not spec.startswith("fzxiezuoai"):
        return False
    rest = spec[6:]
...
def _pin_crewai_deps(content: str, version: str) -> str:
    """Replace fzxiezuoai dependency version pins in a pyproject.toml string.
    ...
    for key in ("dependencies", "optional-dependencies"):
        ...
        for dep_list in dep_lists:
            for i, dep in enumerate(dep_list):
                ...
                dep_list[i] = f"fzxiezuoai{extras}=={version}"
    return tomlkit.dumps(doc)


def update_template_dependencies(templates_dir: Path, new_version: str) -> list[Path]:
    """Update fzxiezuoai dependency versions in CLI template pyproject.toml files.
    ...
    pattern = re.compile(r"(fzxiezuoai(?:\[[\w,]+\])?)(?:==|>=)[^\s\"']+")
    updated = []
...
def _update_all_versions(
    ...
    templates_dir = lib_dir / "cli" / "src" / "fzxiezuoai_cli" / "templates"
...
def _generate_release_notes(
    ...
    no_edit: bool,
    cwd: Path | None = None,
    gh_repo: str = "crewAIInc/12FZ协作AI",
...
def _update_enterprise_crewai_dep(pyproject_path: Path, version: str) -> bool:
    """Update fzxiezuoai pins in an enterprise pyproject.toml.

    Pins ``fzxiezuoai`` / ``fzxiezuoai[extras]`` via ``_pin_crewai_deps`` and
    additionally pins any dashed ``fzxiezuoai-*`` packages configured via
    ``ENTERPRISE_EXTRA_PACKAGES`` (e.g. ``fzxiezuoai-enterprise``), which
    ``_pin_crewai_deps`` does not cover.

    Args:
        pyproject_path: Path to the pyproject.toml file.
        version: New fzxiezuoai version string.
...
def _update_workflow_crewai_pins(workflow_path: Path, version: str) -> bool:
    """Rewrite ``fzxiezuoai[extras]==<old>`` pins in a single workflow file.
...
def _update_enterprise_workflows(repo_dir: Path, version: str) -> list[Path]:
    """Update fzxiezuoai version pins in enterprise CI workflow files.
...
def _update_repo_workflows_crewai_pins(repo_dir: Path, version: str) -> list[Path]:
    """Update fzxiezuoai pins across all GitHub workflow files in a repo.
...
def _repin_crewai_install(run_value: str, version: str) -> str:
    """Rewrite ``fzxiezuoai[extras]==old`` pins in a shell command string.

    Splits on the known ``fzxiezuoai[`` prefix and reconstructs the pin
    ...
    while marker in remainder:
        ...
        if rest.startswith("=="):
            ...
            while ver_end < len(rest) and rest[ver_end] not in ('"', "'", " ", "\n"):
                ver_end += 1
            result.append(f"fzxiezuoai[{extras}]=={version}")
            remainder = rest[ver_end:]
        else:
            result.append(f"fzxiezuoai[{extras}]")
            remainder = rest
...
def _update_deployment_test_repo(version: str, is_prerelease: bool) -> None:
    """Update the deployment test repo to pin the new fzxiezuoai version.

    Clones the repo, updates the fzxiezuoai[tools] pin in pyproject.toml
    and any fzxiezuoai[extras] pins in .github/workflows, regenerates the
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if pyproject_changed:
            pyproject.write_text(new_content)
            console.print(f"[green]✓[/green] Updated fzxiezuoai[tools] pin to {version}")
        else:
            console.print(
                "[yellow]Warning:[/yellow] No fzxiezuoai[tools] pin found to update"
            )

        updated_workflows = _update_repo_workflows_crewai_pins(repo_dir, version)
        for wf in updated_workflows:
            console.print(
                f"[green]✓[/green] Updated fzxiezuoai pin in {wf.relative_to(repo_dir)}"
            )
        ...
        branch = f"chore/bump-fzxiezuoai-v{version}"
        create_or_reset_branch(branch, cwd=repo_dir)

        run_command(["git", "add", *paths_to_add], cwd=repo_dir)
        run_command(
            ["git", "commit", "-m", f"chore: bump fzxiezuoai to {version}"],
            cwd=repo_dir,
        )
        ...
        pr_url = run_command(
            [
                ...
                branch,
                "--title",
                f"chore: bump fzxiezuoai to {version}",
                "--body",
                "",
            ],
            cwd=repo_dir,
...
def _release_enterprise(
    ...
    if dry_run:
        ...
        console.print(
            f"[dim][DRY RUN][/dim] Would update fzxiezuoai[tools] dep in "
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if _update_enterprise_crewai_dep(enterprise_pyproject, version):
            console.print(
                f"[green]✓[/green] Updated fzxiezuoai[tools] dep in {enterprise_dep_path}"
            )
        ...
        _wait_for_pypi("fzxiezuoai", version)
