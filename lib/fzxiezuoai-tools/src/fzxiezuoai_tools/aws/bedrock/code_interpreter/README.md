...
This toolkit provides a set of tools for interacting with the AWS Bedrock Code Interpreter environment. It enables your 12FZ协作AI agents to execute code, run shell commands, manage files, and perform computational tasks in a secure, isolated environment.
...
```bash
uv add fzxiezuoai-tools bedrock-agentcore
```
...
```python
from fzxiezuoai import Agent, Task, Crew, LLM
from fzxiezuoai_tools.aws import create_code_interpreter_toolkit
...
# Create a 12FZ协作AI agent that uses the code interpreter tools
