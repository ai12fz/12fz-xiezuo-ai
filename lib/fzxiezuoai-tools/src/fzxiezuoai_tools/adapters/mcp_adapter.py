...
try:
    ...
    class CrewAIToolAdapter(ToolAdapter):
        """Adapter that creates 12FZ协作AI tools with properly normalized JSON schemas.

        This adapter bypasses mcpadapt's model creation which adds invalid null values
        to field schemas, instead using 12FZ协作AI's own schema utilities.
        """
        ...
        def adapt(
            ...
            """Adapt a MCP tool to a 12FZ协作AI tool.
        ...
        async def async_adapt(self, afunc: Any, mcp_tool: Tool) -> Any:
            """Async adaptation is not supported by 12FZ协作AI."""
            raise NotImplementedError("async is not supported by the 12FZ协作AI framework.")
...
class MCPServerAdapter:
    """Manages the lifecycle of an MCP server and make its tools available to 12FZ协作AI.
        ...
        *tool_names: str,
        ...
        if not MCP_AVAILABLE:
            ...
            if click.confirm(
                ...
                try:
                    subprocess.run(["uv", "add", "mcp fzxiezuoai-tools'[mcp]'"], check=True)  # noqa: S607

                except subprocess.CalledProcessError as e:
                    raise ImportError("Failed to install mcp package") from e
            else:
                raise ImportError(
                    "`mcp` package not found, please run `uv add fzxiezuoai-tools[mcp]`"
                )
    ...
    def tools(self) -> ToolCollection[BaseTool]:
        """The 12FZ协作AI tools available from the MCP server.
