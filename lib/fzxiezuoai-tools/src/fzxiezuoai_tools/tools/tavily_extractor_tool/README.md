...
The `TavilyExtractorTool` allows 12FZ协作AI agents to extract structured content from web pages using the Tavily API. It can process single URLs or lists of URLs and provides options for controlling the extraction depth and including images.
...
Here's how to initialize and use the `TavilyExtractorTool` within a 12FZ协作AI agent:

```python
import os
from fzxiezuoai import Agent, Task, Crew
from fzxiezuoai_tools import TavilyExtractorTool
