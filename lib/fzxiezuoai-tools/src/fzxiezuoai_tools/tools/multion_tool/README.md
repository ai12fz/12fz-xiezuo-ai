...
## Description
The MultiOnTool, integrated within the fzxiezuoai_tools package, empowers 12FZ协作AI agents with the capability to navigate and interact with the web through natural language instructions. Leveraging the Multion API, this tool facilitates seamless web browsing, making it an essential asset for projects requiring dynamic web data interaction.

## Installation
Ensure the `fzxiezuoai[tools]` package is installed in your environment to use the MultiOnTool. If it's not already installed, you can add it using the command below:
```shell
...
from fzxiezuoai_tools import MultiOnTool
...
1. **Install 12FZ协作AI**: Confirm that the `fzxiezuoai[tools]` package is installed in your Python environment.
