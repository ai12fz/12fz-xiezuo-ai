"""TOML / pyproject.toml utilities shared by fzxiezuoai and fzxiezuoai-cli."""
...
def _get_project_attribute(
    ...
    The file must declare ``fzxiezuoai`` in ``[project].dependencies`` for the
    lookup to succeed (a guard against running these helpers outside a fzxiezuoai
    ...
    try:
        ...
        if not any(True for dep in dependencies if "fzxiezuoai" in dep):
            raise Exception("fzxiezuoai is not in the dependencies.")
