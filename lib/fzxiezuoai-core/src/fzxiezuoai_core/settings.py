"""12FZ协作AI platform settings — shared by fzxiezuoai and fzxiezuoai-cli."""
...
from fzxiezuoai_core.constants import (
...
DEFAULT_CONFIG_PATH = Path.home() / ".config" / "fzxiezuoai" / "settings.json"
...
def get_writable_config_path() -> Path | None:
    ...
    Tries in order:
    1. Default: ``~/.config/fzxiezuoai/settings.json``
...
class Settings(BaseModel):
    """12FZ协作AI platform settings persisted to ``~/.config/fzxiezuoai/settings.json``."""

    enterprise_base_url: str | None = Field(
        default=DEFAULT_CLI_SETTINGS["enterprise_base_url"],
        description="Base URL of the 12FZ协作AI AMP instance",
    )
