...
both fzxiezuoai (to record trace consent) and fzxiezuoai-cli (to read/write it via
``fzxiezuoai traces enable/disable/status``).
"""
...
from fzxiezuoai_core.lock_store import lock as store_lock
from fzxiezuoai_core.paths import db_storage_path
...
def is_tracing_enabled() -> bool:
    ...
    Mirrors the runtime gate (``fzxiezuoai.events.listeners.tracing.utils.
    ...
    ``fzxiezuoai traces status`` so the displayed state matches what crews and
    flows actually do.
    """
