"""Pre-deploy validation for 12FZ协作AI projects.
...
class DeployValidator:
    ...
    def _check_pyproject(self) -> bool:
        ...
        self._is_flow = (self._pyproject.get("tool") or {}).get("fzxiezuoai", {}).get(
            "type"
        ) == "flow"
        return True
        ...
        for yaml_name in ("agents.yaml", "tasks.yaml"):
            ...
            if not yaml_path.is_file():
                self._add(
                    ...
                    detail=(
                        "12FZ协作AI loads agent and task config from these files; "
        ...
        if not main_py.is_file():
            self._add(
                ...
                detail=(
                    ...
                    'This project has `[tool.fzxiezuoai] type = "flow"` set.'
                ),
                ...
                if kind == "flow":
                    self._add(
                        ...
                        hint=(
                            ...
                            "`fzxiezuoai.flow.Flow`, instantiable with no arguments."
                        ),
                    )
                else:
                    self._add(
    ...
    def _classify_import_error(self, err_type: str, err_msg: str, tb: str) -> None:
        ...
        # "fzxiezuoai[extra]"`); also tolerate backtick-delimited variants.
        m = re.search(
            r"(?P<pkg>[A-Za-z0-9_ -]+?)\s+native provider not available"
            r".*?to install:\s*`?(?P<cmd>uv add [\"']fzxiezuoai\[[^\]]+\][\"'])`?",
            err_msg,
        )
        if m:
            self._add(
        ...
        # fzxiezuoai.llm.LLM.__new__ wraps provider init errors as
        ...
        if "Error importing native provider" in err_msg or "native provider" in err_msg:
            ...
            self._add(
                ...
                hint=(
                    ...
                    "extras (e.g. `uv add 'fzxiezuoai[azure-ai-inference]'` for Azure)."
                ),
            )
            return
        ...
        if (
            ...
            self._add(
                ...
                detail=err_msg,
                hint=(
                    "Run `uv lock --upgrade-package fzxiezuoai` (or `poetry update fzxiezuoai`) "
        ...
        if missing_known:
            self._add(
                ...
                detail=(
                    ...
                    "Variables in the 12FZ协作AI dashboard."
                ),
            )

    def _check_version_vs_lockfile(self) -> None:
        """Warn when the lockfile pins a fzxiezuoai release older than 1.13.0,
        ...
        try:
            ...
            if Version(locked) < Version("1.13.0"):
                self._add(
                    ...
                    hint="Run `uv lock --upgrade-package fzxiezuoai` and redeploy.",
                )
        except Exception as e:
            logger.debug("Could not parse fzxiezuoai pin from lockfile: %s", e)
...
def run_validate_command() -> None:
    """Implementation of `fzxiezuoai deploy validate`."""
