...
from fzxiezuoai_cli.utils import (
...
_REQUIRES_CREWAI_MSG = (
    "[red]This subcommand requires the full fzxiezuoai package.\n"
    "Install it with: pip install fzxiezuoai[/red]"
)


def _require_project_utils() -> Any:
    try:
        from fzxiezuoai.utilities import project_utils
...
def _require_get_user_id() -> Any:
    try:
        from fzxiezuoai.events.listeners.tracing.utils import get_user_id
...
class ToolCommand(BaseCommand, PlusAPIMixin):
    """
    A class to handle tool repository related operations for 12FZ协作AI projects.
    """
    ...
    def login(self) -> None:
        ...
        if login_response.status_code != 200:
            console.print(
                "Authentication failed. Verify if the currently active organization can access the tool repository, and run 'fzxiezuoai login' again.",
                style="bold red",
            )
            try:
                console.print(
