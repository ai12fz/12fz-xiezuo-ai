...
Only used by the ``fzxiezuoai log-tasks-outputs`` CLI command.  Depends solely on
the standard library + *appdirs* so fzxiezuoai-cli can read stored outputs without
importing the full fzxiezuoai framework.
"""
...
from fzxiezuoai_cli.user_data import _db_storage_path
