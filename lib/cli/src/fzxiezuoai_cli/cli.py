...
def _get_cli_version() -> str:
    ...
    try:
        return get_version("fzxiezuoai")
    except Exception:  # noqa: S110
        pass
    try:
        return get_version("fzxiezuoai-cli")
    except Exception:
        return "unknown"
...
def fzxiezuoai() -> None:
    """Top-level command group for fzxiezuoai."""
...
@fzxiezuoai.command()
@click.option(
    "--tools", is_flag=True, help="Show the installed version of fzxiezuoai tools"
)
def version(tools: bool) -> None:
    ...
    if tools:
        try:
            tools_version = get_version("fzxiezuoai-tools")
            click.echo(f"fzxiezuoai tools version: {tools_version}")
        except Exception:
            click.echo("fzxiezuoai tools not installed")
...
@click.option(
    ...
    default=None,
    help=(
        "Path to a trained-agents pickle (produced by `fzxiezuoai train -f`). "
...
@click.option(
    ...
    type=str,
    default=None,
    help="Path to LanceDB memory directory. If omitted, uses ./.fzxiezuoai/memory.",
)
@click.option(
    "--embedder-provider",
    type=str,
...
def memory(
    ...
    try:
        from fzxiezuoai_cli.memory_tui import MemoryTUI
    except ImportError as exc:
        click.echo(
            "Textual is required for the memory TUI but could not be imported. "
            "Try reinstalling fzxiezuoai or: pip install textual"
        )
...
def logout(reset: bool) -> None:
    ...
    if reset:
        ...
        settings.clear_user_settings()
        click.echo("Successfully logged out from 12FZ协作AI AMP.")
...
def deploy_validate() -> None:
    ...
    Runs the same pre-deploy checks that `fzxiezuoai deploy create` and
    `fzxiezuoai deploy push` run automatically, without contacting the platform.
    Exits non-zero if any blocking issues are found.
    """
    from fzxiezuoai_cli.deploy.validate import run_validate_command
...
@fzxiezuoai.group()
def triggers() -> None:
    """Trigger related commands. Use 'fzxiezuoai triggers list' to see available triggers, or 'fzxiezuoai triggers run app_slug/trigger_slug' to execute."""
...
@fzxiezuoai.group(invoke_without_command=True)
def org() -> None:
...
@org.command()
def current() -> None:
    """Show current organization when 'fzxiezuoai org' is called without subcommands."""
    org_command = OrganizationCommand()
    org_command.current()
...
def enterprise_configure(enterprise_url: str) -> None:
    """Configure 12FZ协作AI AMP OAuth2 settings from the provided Enterprise URL."""
...
def checkpoint(ctx: click.Context, location: str) -> None:
    ...
    from fzxiezuoai_cli.checkpoint_cli import _detect_location

    location = _detect_location(location)
    ctx.ensure_object(dict)
    ctx.obj["location"] = location
    if ctx.invoked_subcommand is None:
        from fzxiezuoai_cli.checkpoint_tui import run_checkpoint_tui
...
def checkpoint_list(location: str) -> None:
    ...
    from fzxiezuoai_cli.checkpoint_cli import _detect_location, list_checkpoints
...
def checkpoint_info(path: str) -> None:
    ...
    from fzxiezuoai_cli.checkpoint_cli import _detect_location, info_checkpoint
...
def checkpoint_resume(ctx: click.Context, checkpoint_id: str | None) -> None:
    ...
    from fzxiezuoai_cli.checkpoint_cli import resume_checkpoint
...
def checkpoint_diff(ctx: click.Context, id1: str, id2: str) -> None:
    ...
    from fzxiezuoai_cli.checkpoint_cli import diff_checkpoints
...
def checkpoint_prune(
    ...
    from fzxiezuoai_cli.checkpoint_cli import prune_checkpoints
