...
def install_xiezuo(proxy_options: list[str]) -> None:
    """
    Install the xiezuo by running the UV command to lock and install.
    """
    try:
        command = ["uv", "sync", *proxy_options]

        # Inject tool repository credentials so uv can authenticate
        # against private package indexes (e.g. fzxiezuoai tool repository).
        ...
        click.echo(f"An error occurred while running the xiezuo: {e}", err=True)
