"""Skill Repository CLI commands for 12FZ协作AI."""
...
from fzxiezuoai_cli.constants import DEFAULT_CREWAI_ENTERPRISE_URL
...
class SkillCommand(BaseCommand, PlusAPIMixin):
    """Skill Repository related operations for 12FZ协作AI projects."""
    ...
    def install(self, ref: str) -> None:
        ...
        if in_project:
            ...
            try:
                from fzxiezuoai.experimental.skills.cache import SkillCacheManager

                cache = SkillCacheManager()
                cache.store(org, name, version, archive_bytes)
            except ImportError:
                # Fallback if SDK not installed — write directly
                cache_dir = Path.home() / ".fzxiezuoai" / "skills" / org / name
                if cache_dir.exists():
                    import shutil
                ...
                # Write metadata so `fzxiezuoai skill list` can discover it
    ...
    def publish(self, is_public: bool, org: str | None, force: bool = False) -> None:
        ...
        if not effective_org:
            console.print(
                "[red]No organisation set. Run `fzxiezuoai org switch <org_id>` first, "
                "or pass --org.[/red]"
            )
            raise SystemExit(1)
    ...
    def list_cached(self) -> None:
        ...
        cache_root = Path.home() / ".fzxiezuoai" / "skills"
    ...
    def _print_current_organization(self) -> None:
        ...
        if settings.org_uuid:
            ...
            console.print(
                ...
                "`fzxiezuoai org switch <org_id>` command.",
                style="yellow",
            )
    ...
    def _parse_frontmatter(self, content: str) -> dict[str, str]:
        ...
        Reuses fzxiezuoai.skills.parser when available, with a minimal
        fallback for environments where the full SDK isn't installed.
        """
        try:
            from fzxiezuoai.skills.parser import parse_frontmatter
