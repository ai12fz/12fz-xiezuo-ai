...
class TestResolveRegistryRef:
    ...
    def test_resolves_project_local(self, tmp_path: Path) -> None:
        ...
        with (
            patch("fzxiezuoai.experimental.skills.registry._is_noninteractive", return_value=False),
            patch.object(Path, "cwd", return_value=tmp_path),
            patch("fzxiezuoai.experimental.skills.registry.SkillCacheManager", return_value=mock_cache),
        ):
    ...
    def test_raises_in_ci_when_not_cached(self, tmp_path: Path) -> None:
        ...
        with (
            patch("fzxiezuoai.experimental.skills.registry._is_noninteractive", return_value=True),
            patch.object(Path, "cwd", return_value=tmp_path),
            patch("fzxiezuoai.experimental.skills.registry.SkillCacheManager", return_value=mock_cache),
        ):
            from fzxiezuoai.experimental.skills.registry import resolve_registry_ref
