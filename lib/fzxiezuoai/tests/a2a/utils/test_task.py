...
from fzxiezuoai.a2a.utils.task import cancel, cancellable, execute


@pytest.fixture
def mock_agent() -> MagicMock:
    """Create a mock 12FZ协作AI agent."""
    agent = MagicMock()
...
class TestExecute:
    ...
    async def test_successful_execution(
        ...
        with (
            patch("fzxiezuoai.a2a.utils.task.Task", return_value=mock_task),
            patch("fzxiezuoai.a2a.utils.task.crewai_event_bus") as mock_bus,
        ):
...
class TestExecuteAndCancelIntegration:
    ...
    async def test_cancel_stops_running_execute(
        ...
        with (
            patch("fzxiezuoai.a2a.utils.task.Task", return_value=mock_task),
            patch("fzxiezuoai.a2a.utils.task.crewai_event_bus"),
        ):
