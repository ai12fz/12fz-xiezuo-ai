...
from fzxiezuoai.flow.constants import AND_CONDITION, OR_CONDITION
from fzxiezuoai.flow.dsl._conditions import (
...
from fzxiezuoai.flow.flow_context import current_flow_id, current_flow_request_id
from fzxiezuoai.flow.flow_definition import FlowDefinition
from fzxiezuoai.flow.flow_wrappers import (
    FlowCondition,
    FlowMethod,
    ListenMethod,
    RouterMethod,
...
from fzxiezuoai.flow.human_feedback import HumanFeedbackResult
from fzxiezuoai.flow.input_provider import InputProvider
from fzxiezuoai.flow.persistence.base import FlowPersistence
from fzxiezuoai.flow.types import (
    FlowExecutionData,
    FlowMethodName,
    InputHistoryEntry,
    PendingListenerKey,
...
from fzxiezuoai.state.checkpoint_config import (
    CheckpointConfig,
    _coerce_checkpoint,
    apply_checkpoint,
)
...
if TYPE_CHECKING:
    from fzxiezuoai_files import FileInput

    from fzxiezuoai.context import ExecutionContext
    from fzxiezuoai.flow.async_feedback.types import PendingFeedbackContext
    from fzxiezuoai.llms.base_llm import BaseLLM

from fzxiezuoai.flow.visualization import build_flow_structure, render_interactive
from fzxiezuoai.types.streaming import CrewStreamingOutput, FlowStreamingOutput
from fzxiezuoai.utilities.env import get_env_context
...
# ``fzxiezuoai.context.ExecutionContext`` themselves. The real class is brought
# in under ``TYPE_CHECKING`` above for static analysis. We can't import it at
# runtime because ``fzxiezuoai.context`` is loaded mid-initialization when this
# module is imported through ``fzxiezuoai.__init__`` (circular).
...
def _resolve_persistence(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        from fzxiezuoai.flow.persistence.base import _persistence_registry
...
def _validate_input_provider(value: Any) -> Any:
    if value is None or isinstance(value, InputProvider):
        return value
    from fzxiezuoai.types.callback import _dotted_path_to_instance
...
def _serialize_input_provider(value: Any) -> str | None:
    if value is None:
        return None
    from fzxiezuoai.types.callback import _instance_to_dotted_path
...
def _deserialize_initial_state(value: Any) -> Any:
    ...
    if isinstance(value, dict) and _INITIAL_STATE_CLASS_MARKER in value:
        from fzxiezuoai.utilities.pydantic_schema_utils import create_model_from_schema
...
class Flow(_ConversationalMixin, BaseModel, Generic[T], metaclass=FlowMeta):
    ...
    # built-in graph + helpers) lives under ``fzxiezuoai.experimental`` and may
    # change shape before graduating. Pin your 12FZ协作AI version if you depend on
    ...
    def _flow_post_init(self) -> None:
        ...
        if self.memory is None and not getattr(self, "_skip_auto_memory", False):
            from fzxiezuoai.memory.utils import sanitize_scope_name
    ...
    async def resume_async(self, feedback: str = "") -> Any:
        ...
        if method is not None:
            ...
            if live_llm is not None:
                from fzxiezuoai.llms.base_llm import BaseLLM as BaseLLMClass
        ...
        try:
            ...
            from fzxiezuoai.flow.async_feedback.types import HumanFeedbackPending
    ...
    async def _execute_method(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            from fzxiezuoai.flow.flow_context import current_flow_method_name
    ...
    def ask(
        ...
        from fzxiezuoai.flow.flow_context import current_flow_method_name
        from fzxiezuoai.flow.input_provider import InputResponse
