...
workflows within 12FZ协作AI Flows. It allows collecting human feedback on method outputs
...
Example (synchronous, default):
    ```python
    from fzxiezuoai.flow import Flow, start, listen, human_feedback
...
Example (asynchronous with custom provider):
    ```python
    from fzxiezuoai.flow import Flow, start, human_feedback
    from fzxiezuoai.flow.async_feedback import HumanFeedbackProvider, HumanFeedbackPending
...
from fzxiezuoai.flow.flow_wrappers import FlowMethod


if TYPE_CHECKING:
    from fzxiezuoai.flow.async_feedback.types import HumanFeedbackProvider
    ...
    if emit is not None:
        if not llm:
            raise ValueError(
                ...
                "See the 12FZ协作AI Human-in-the-Loop (HITL) documentation for more information: "
                "https://docs.fzxiezuoai.com/en/learn/human-feedback-in-flows"
            )
...
def human_feedback(
    ...
    from fzxiezuoai.flow.dsl._human_feedback import human_feedback as dsl_human_feedback
