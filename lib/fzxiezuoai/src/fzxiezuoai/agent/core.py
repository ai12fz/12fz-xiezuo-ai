"""Core agent implementation for the 12FZ协作AI framework."""
...
from fzxiezuoai.agent.utils import (
...
from fzxiezuoai.agents.agent_builder.base_agent import (
    BaseAgent,
    _serialize_llm_ref,
    _validate_llm_ref,
)
...
from fzxiezuoai.events.types.skill_events import SkillActivatedEvent
...
from fzxiezuoai.skills.models import INSTRUCTIONS, Skill as SkillModel
from fzxiezuoai.state.checkpoint_config import CheckpointConfig, apply_checkpoint
from fzxiezuoai.tools.agent_tools.agent_tools import AgentTools
...
from fzxiezuoai.utilities.constants import (
...
from fzxiezuoai.utilities.guardrail import process_guardrail, serialize_guardrail_for_json
from fzxiezuoai.utilities.guardrail_types import GuardrailCallable, GuardrailType
from fzxiezuoai.utilities.i18n import I18N_DEFAULT
from fzxiezuoai.utilities.llm_utils import create_llm
from fzxiezuoai.utilities.prompts import Prompts, StandardPromptResult, SystemPromptResult
from fzxiezuoai.utilities.pydantic_schema_utils import generate_model_description
...
try:
    from fzxiezuoai.a2a.types import AgentResponseProtocol
except ImportError:
...
if TYPE_CHECKING:
    ...
    from fzxiezuoai.a2a.config import A2AClientConfig, A2AConfig, A2AServerConfig
    from fzxiezuoai.agents.agent_builder.base_agent import PlatformAppOrAction
...
def _validate_executor_class(value: Any) -> Any:
    ...
    if value is CrewAgentExecutor:
        warnings.warn(
            ...
            "Switch to fzxiezuoai.experimental.AgentExecutor.",
            DeprecationWarning,
            stacklevel=3,
        )
    return value
...
class Agent(BaseAgent):
    ...
    Attributes:
            ...
            apps: List of applications that the agent can access through 12FZ协作AI Platform.
    ...
    def set_skills(
        ...
        for item in items:
            if isinstance(item, str):
                from fzxiezuoai.experimental.skills.registry import (
                    is_registry_ref,
                    parse_registry_ref,
    ...
    def get_mcp_tools(self, mcps: list[str | MCPServerConfig]) -> list[BaseTool]:
        """Convert MCP server references/configs to 12FZ协作AI tools.

        Delegates to :class:`~fzxiezuoai.mcp.tool_resolver.MCPToolResolver`.
        """
        self._cleanup_mcp_clients()
        from fzxiezuoai.mcp.tool_resolver import MCPToolResolver
    ...
    def _prepare_kickoff(
        ...
        if agent_memory is not None:
            from fzxiezuoai.tools.memory_tools import create_memory_tools
    ...
    def kickoff(
        ...
        Args:
            ...
            response_format: Optional Pydantic model for structured output.
            input_files: Optional dict of named files to attach to the message.
                   Files can be paths, bytes, or File objects from fzxiezuoai_files.
