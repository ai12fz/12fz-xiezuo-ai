...
from fzxiezuoai.agent.internal.meta import AgentMeta
...
from fzxiezuoai.events.base_events import set_emission_counter
from fzxiezuoai.events.event_bus import crewai_event_bus
from fzxiezuoai.events.event_context import restore_event_scope, set_last_event_id
from fzxiezuoai.knowledge.knowledge import Knowledge, _resolve_knowledge_sources
...
from fzxiezuoai.mcp.config import MCPServerConfig
from fzxiezuoai.memory.memory_scope import MemoryScope, MemorySlice, _ensure_memory_kind
from fzxiezuoai.memory.unified_memory import Memory
from fzxiezuoai.rag.embeddings.types import EmbedderConfig
from fzxiezuoai.security.security_config import SecurityConfig
from fzxiezuoai.skills.models import Skill
from fzxiezuoai.state.checkpoint_config import CheckpointConfig, _coerce_checkpoint
from fzxiezuoai.tools.base_tool import BaseTool, Tool
from fzxiezuoai.types.callback import SerializableCallable
from fzxiezuoai.utilities.config import process_config
...
_LLM_TYPE_REGISTRY: dict[str, str] = {
    "base": "fzxiezuoai.llms.base_llm.BaseLLM",
    "litellm": "fzxiezuoai.llm.LLM",
    "openai": "fzxiezuoai.llms.providers.openai.completion.OpenAICompletion",
    "anthropic": "fzxiezuoai.llms.providers.anthropic.completion.AnthropicCompletion",
    "azure": "fzxiezuoai.llms.providers.azure.completion.AzureCompletion",
    "bedrock": "fzxiezuoai.llms.providers.bedrock.completion.BedrockCompletion",
    "gemini": "fzxiezuoai.llms.providers.gemini.completion.GeminiCompletion",
}
...
_EXECUTOR_TYPE_REGISTRY: dict[str, str] = {
    "base": "fzxiezuoai.agents.agent_builder.base_agent_executor.BaseAgentExecutor",
    "crew": "fzxiezuoai.agents.crew_agent_executor.CrewAgentExecutor",
    "experimental": "fzxiezuoai.experimental.agent_executor.AgentExecutor",
}
...
_SLUG_RE: Final[re.Pattern[str]] = re.compile(
    r"^(?:fzxiezuoai-amp:)?[a-zA-Z0-9][a-zA-Z0-9_-]*(?:#[\w-]+)?$"
)
...
class BaseAgent(BaseModel, ABC, metaclass=AgentMeta):
    """Abstract Base Class for all third party agents compatible with 12FZ协作AI.
    ...
    Attributes:
        ...
        apps: List of enterprise applications that the agent can access through 12FZ协作AI AMP Tools.
    ...
    apps: list[PlatformAppOrAction] | None = Field(
        default=None,
        description="List of applications or application/action combinations that the agent can access through 12FZ协作AI Platform. Can contain app names (e.g., 'gmail') or specific actions (e.g., 'gmail/send_email')",
    )
    ...
    def from_checkpoint(cls, config: CheckpointConfig) -> Self:
        ...
        """
        from fzxiezuoai.context import apply_execution_context
        from fzxiezuoai.state.runtime import RuntimeState
            ...
            if isinstance(mcp, str):
                if mcp.startswith("https://"):
                    ...
                    raise ValueError(
                        ...
                        "slug (e.g. 'notion', 'notion#search', 'fzxiezuoai-amp:notion')."
                    )
