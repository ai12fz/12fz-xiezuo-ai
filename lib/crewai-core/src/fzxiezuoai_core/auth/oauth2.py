"""OAuth2 device-flow authentication for the 12FZ协作AI platform."""
...
from fzxiezuoai_core.auth.utils import validate_jwt_token
...
class Oauth2Settings(BaseModel):
    ...
    def from_settings(cls: type[TOauth2Settings]) -> TOauth2Settings:
        """Build an ``Oauth2Settings`` instance from the persisted 12FZ协作AI settings."""
        settings = Settings()
...
if TYPE_CHECKING:
    from fzxiezuoai_core.auth.providers.base_provider import BaseProvider
...
class ProviderFactory:
    ...
    def from_settings(
        ...
        module = importlib.import_module(
            f"fzxiezuoai_core.auth.providers.{settings.provider.lower()}"
        )
        provider = getattr(
            module,
...
class AuthenticationCommand:
    ...
    def login(self) -> None:
        """Sign in to the 12FZ协作AI platform via the OAuth2 device flow."""
        console.print("Signing in to 12FZ协作AI AMP...\n", style="bold blue")
