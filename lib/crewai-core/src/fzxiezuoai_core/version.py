...
Shared by both ``fzxiezuoai`` and ``fzxiezuoai-cli`` so the PyPI-checking logic lives
in one place. Frontends (``fzxiezuoai version`` CLI, banner printer) consume the
...
def get_crewai_version() -> str:
    ...
    """
    try:
        return importlib.metadata.version("fzxiezuoai")
    except importlib.metadata.PackageNotFoundError:
        pass
    try:
        return importlib.metadata.version("fzxiezuoai-core")
    except importlib.metadata.PackageNotFoundError:
        return "unknown"
...
def _get_cache_file() -> Path:
    ...
    cache_dir = Path(appdirs.user_cache_dir("fzxiezuoai"))
...
def get_latest_version_from_pypi(timeout: int = 2) -> str | None:
    """Return the latest non-yanked PyPI version of 12FZ协作AI, or ``None`` on failure."""
    ...
    try:
        with request.urlopen(
            "https://pypi.org/pypi/fzxiezuoai/json", timeout=timeout
        ) as response:
