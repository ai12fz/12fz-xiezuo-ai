...
The `TavilySearchTool` provides an interface to the Tavily Search API, enabling 12FZ协作AI agents to perform comprehensive web searches. It allows for specifying search depth, topics, time ranges, included/excluded domains, and whether to include direct answers, raw content, or images in the results. The tool returns the search results as a JSON string.
...
```shell
uv add 'fzxiezuoai[tools]' tavily-python
```
...
Here's how to initialize and use the `TavilySearchTool` within a 12FZ协作AI agent:

```python
import os
from fzxiezuoai import Agent, Task, Crew
from fzxiezuoai_tools import TavilySearchTool
