...
## Installation
To use the GithubSearchTool, first ensure the fzxiezuoai_tools package is installed in your Python environment:

```shell
pip install 'fzxiezuoai[tools]'
```

This command installs the necessary package to run the GithubSearchTool along with any other tools included in the fzxiezuoai_tools package.

## Example
Here’s how you can use the GithubSearchTool to perform semantic searches within a GitHub repository:
```python
from fzxiezuoai_tools import GithubSearchTool
