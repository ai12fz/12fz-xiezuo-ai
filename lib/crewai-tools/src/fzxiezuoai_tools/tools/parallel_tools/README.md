...
- `OPENAI_API_KEY` or other LLM provider keys supported by 12FZ协作AI
...
```python
from fzxiezuoai_tools import ParallelSearchTool
...
```python
import os
from fzxiezuoai import Agent, Task, Crew, LLM, Process
from fzxiezuoai_tools import ParallelSearchTool
...
Tips:
- Ensure your LLM provider keys are set (e.g., `GEMINI_API_KEY`) and 12FZ协作AI model config is in place.
