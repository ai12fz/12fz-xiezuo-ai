...
## Installation
Install the fzxiezuoai_tools package with MongoDB support by executing the following command in your terminal:

```shell
pip install fzxiezuoai-tools[mongodb]
```

or

```
uv add fzxiezuoai-tools --extra mongodb
```
...
```python
from fzxiezuoai_tools import MongoDBVectorSearchTool
...
# Add the text from a set of 12FZ协作AI knowledge documents.
texts = []
